...
export function MinistriesView({ isEn = false }: { isEn?: boolean }) {
  const [activeTab, setActiveTab] = useState<"kids-students" | "adults" | "serve" | "care">("kids-students");

  const tabs = [
    { id: "kids-students", label: isEn ? "Kids & Students" : "சிறுவர் & மாணவர்" },
    { id: "adults", label: isEn ? "Adult Groups" : "பெரியவர்கள்" },
    { id: "serve", label: isEn ? "Worship & Serving" : "ஆராதனை & சேவை" },
    { id: "care", label: isEn ? "Pastoral Care" : "பராமரிப்பு" }
  ];
    ...
    <div className="pb-20 space-y-16">
      ...
      >
        ...
        <div className="max-w-4xl mx-auto px-4 relative z-10 space-y-3">
          <h1 className="font-serif text-4xl md:text-5xl font-black">{isEn ? "Our Ministries" : "நமது ஊழியங்கள்"}</h1>
          <p className="text-stone-300 text-base md:text-lg max-w-xl mx-auto font-light leading-relaxed">
            {isEn ? "Discover the active communities, classes, and outreach programs where you can connect and serve." : "நீங்கள் இணைந்து சேவையாற்றக்கூடிய பல்வேறு ஊழியங்கள் மற்றும் குழுக்களைக் குறித்து அறிந்துகொள்ளுங்கள்."}
          </p>
        </div>
      </section>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {filteredMinistries.map((ministry) => (
            <MinistryCard key={ministry.id} ministry={ministry} basePrefix={isEn ? "/en" : ""} />
          ))}
        </div>
      </section>
