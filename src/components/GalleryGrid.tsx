    ...
    <div className="space-y-16">
      {folders.length === 0 ? (
        ...
        folders.map((folder, folderIdx) => (
          <div key={folder.name} className="space-y-6">
            <h2 className="text-2xl font-serif font-bold border-b border-border pb-2 text-stone-800 flex items-center justify-between">
              <span>{folder.name}</span>
              {folder.images.length > 0 && (
                <span className="text-xs font-sans font-normal text-stone-500 bg-stone-200/60 px-3 py-1 rounded-full">
            ...
            {folder.images.length === 0 ? (
              ...
              <div className="columns-1 sm:columns-2 lg:columns-3 gap-6 space-y-6">
                {folder.images.map((imgSrc, imgIdx) => {
                  const isLiked = !!likedPhotos[imgSrc];
                  return (
                    <div
                      key={imgSrc}
                      onClick={() => setLightboxIndex({ folderIdx, imgIdx })}
                    ...
                    >
                      <div className="relative overflow-hidden">
                        <img
                          src={imgSrc}
                          alt={`${folder.name} photo ${imgIdx + 1}`}
