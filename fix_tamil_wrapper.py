...
start_marker = '<h3 className="text-xl font-bold text-stone-800">Sharpen the Axe</h3>'
end_marker = '</ul>\n                </div>\n\n              </div>\n            </div>\n          </article>'
...
if start_idx == -1 or end_idx == -1:
    ...
    article_content = article_content.replace(start_marker, "") # we will add the header manually
...
"""
    ...
    # Wait, the end of the article has "ஆசீர்வாதங்களுடன், பாஸ்டர் வெஸ்லின்", which we want to remove because the author block is already there.
    new_html = new_html.replace('<p className="mt-8 font-bold">ஆசீர்வாதங்களுடன்,<br/>பாஸ்டர் வெஸ்லின்<br/>வார்த்தை சுவிசேஷ சபை</p>', '')
